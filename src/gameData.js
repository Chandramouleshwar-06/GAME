...
// Map for easy lookup of mastery values for Nexus Rating, Dodge, and Damage Bonus
...
// Map for easy lookup of special ability values for Nexus Rating based on rarity/weight
// Toned down values further to make high ranks less common
export const specialAbilityNexusValues = {
  ...
  8: 1,
  9: 0, // (was 1)
  10: 0, // Commonest (was 0)
};

export const fatalFlaws = [
  { name: "None", combatEffect: null },
  { name: "Arrogance", combatEffect: { selfDamageChance: 0.1, selfDamageAmount: 5 } }, // 10% chance to take 5 self damage per turn
  { name: "Impulsiveness", combatEffect: { forcedAttackChance: 0.15 } }, // 15% chance to be forced to attack
  { name: "Fear of Water", combatEffect: { opponentDamageBoost: 0.1, appliesToType: "water" } }, // Opponent's Water type attacks deal 10% more damage
  { name: "Fragile Ego", combatEffect: { critReceivedBoost: 0.1 } }, // +10% chance for opponent to crit against you
  { name: "Naivety", combatEffect: { debuffDurationBoost: 1 } }, // Debuffs last 1 turn longer on you
  { name: "Curse of Weakness", combatEffect: { statPenalty: { Strength: -5, CombatSkill: -5 }, duration: -1 } }, // Permanent in-combat stat penalty
  { name: "Dark Secret", combatEffect: { defenseReductionChance: 0.05, defenseReductionAmount: 0.2, duration: 1 } }, // 5% chance to lose 20% defense for 1 turn
  { name: "Overconfidence", combatEffect: { abilityFailChance: 0.1 } }, // 10% chance for ability to fail
  { name: "Kleptomania", combatEffect: { lootDropPenalty: 0.2 } }, // 20% reduced loot from battles (post-combat effect)
  { name: "Compulsive Liar", combatEffect: { socialSkillPenalty: 0.1 } }, // Affects future quest social checks (post-combat effect)
  { name: "Claustrophobia", combatEffect: { environmentPenalty: "confined", statPenalty: { Speed: -10 }, duration: -1 } }, // Speed penalty in confined spaces
  { name: "Fear of Heights", combatEffect: { environmentPenalty: "high", statPenalty: { CombatSkill: -10 }, duration: -1 } } // Combat Skill penalty in high places
];
...
// Nexus Rank Thresholds (for progression and rank-up quests)
...
export const levelUpThresholds = {
  ...
  8: 2200,
  9: 2700,
  10: 3250, // Max Level for now, can expand
};

// Item Definitions (Placeholder examples)
export const itemTypes = {
  ...
  "stat_point_item": [ // Represents a reward that gives floating stat points
...
// Function for weighted random choice (for array of {name, weight} objects)
...
export const questTemplates = [
  {
    ...
    progress: { currentWins: 0 } // New: tracks progress
  },
  {
    type: "Battle Quest",
    name: "Goblin Menace",
