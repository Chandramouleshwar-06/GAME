// src/components/CombatArena.jsx
import React, { useState, useEffect, useCallback, useRef } from 'react';
import { ChevronRight, Sword, Shield, Zap } from 'lucide-react';
import { fatalFlaws, weaponTypes, weaponMasteryValues, specialAbilities, statNames } from '../gameData.js'; // Explicit .js extension
import { randomInt } from '../utils/helpers.js'; // Explicit .js extension
...
const CombatArena = ({ player, opponent, onClose, onCombatEnd, activeQuest, updateQuestProgress }) => {
  ...
  // Function to add messages to combat log
  ...
  const calculatePhysicalDamage = useCallback((attacker, defender, attackTypeElement = null) => {
    ...
    const defenderEffectiveStats = getEffectiveStats(defender.Stats, defender.Payload.activeBuffs);
  ...
  // Combat System Helper: Dodge Chance Calculation
  ...
  const handleTurn = useCallback(async (action, abilityName = null) => {
    ...
    // Decrement cooldowns and buff/debuff durations for the character whose turn it IS
    ...
    // Apply Fatal Flaw effects that trigger at start of turn
      ...
      <div className="bg-gray-800 rounded-xl shadow-2xl p-6 border-2 border-red-700 w-full max-w-4xl h-5/6 flex flex-col">
        ...
        <div className="grid grid-cols-2 gap-4 flex-grow overflow-hidden">
          ...
          <div className={`bg-gray-900 rounded-lg p-4 flex flex-col items-center border ${currentAttackerId === 'player' ? 'border-yellow-400' : 'border-purple-700'}`}>
            <div className="w-24 h-24 bg-purple-500 rounded-full mb-2 flex items-center justify-center text-xl font-bold">You</div>
          ...
          <div className={`bg-gray-900 rounded-lg p-4 flex flex-col items-center border ${currentAttackerId === 'opponent' ? 'border-yellow-400' : 'border-red-700'}`}>
            <div className="w-24 h-24 bg-red-500 rounded-full mb-2 flex items-center justify-center text-xl font-bold">AI</div>
